...
while($row = $qry->fetch_assoc()){
	$from_location = $conn->query("SELECT id,Concat(Starting_from,', ',seats,',',status) as location FROM location where id = ".$row['from_location'])->fetch_array()['location'];
	$to_location = $conn->query("SELECT id,Concat(Destination,',',seats,',',status) as location FROM location where id = ".$row['to_location'])->fetch_array()['location'];
