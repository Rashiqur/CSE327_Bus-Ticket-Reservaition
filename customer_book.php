...
$from_location = $conn->query("SELECT id,Concat(Starting_from,', ','seats,',',status) as location FROM location where id =".$meta['from_location'])->fetch_array();
$to_location = $conn->query("SELECT id,Concat(Destination,', ','seats,',',status) as location FROM location where id =".$meta['to_location'])->fetch_array();
